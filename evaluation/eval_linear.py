...
def resolve_split_dir(data_path, split_name, fallback_split=None):
    split_dir = os.path.join(data_path, split_name)
    if os.path.isdir(split_dir):
        return split_dir
    if fallback_split is not None:
        fallback_dir = os.path.join(data_path, fallback_split)
        if os.path.isdir(fallback_dir):
            print(f"Split '{split_name}' not found under {data_path}, using '{fallback_split}' instead.")
            return fallback_dir
...
def eval_linear(args):
    ...
    traindir = resolve_split_dir(args.data_path, args.train_split)
    valdir = resolve_split_dir(args.data_path, args.val_split, fallback_split="test" if args.val_split == "val" else None)
    ...
    if args.num_labels <= 0:
        ...
        print(f"Inferred num_labels={args.num_labels} from {traindir}.")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_split', default='train', type=str,
        help='Training split folder name under data_path.')
    parser.add_argument('--val_split', default='val', type=str,
        help='Validation split folder name under data_path. Use test for datasets like CUB.')
