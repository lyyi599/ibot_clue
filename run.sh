...
if [ -z "$FGVC_DATASET" ]; then
    if [[ $TYPE =~ cub_ ]]; then
        FGVC_DATASET=cub
    elif [[ $TYPE =~ air_ ]]; then
        FGVC_DATASET=air
    elif [[ $TYPE =~ car_ ]] || [[ $TYPE =~ cars_ ]]; then
        if [ -d "$CURDIR/dataset/cars" ]; then
            FGVC_DATASET=cars
        else
            FGVC_DATASET=car
        fi
...
if [ -z "$FGVC_DATA_ROOT" ] && [ ! -z "$FGVC_DATASET" ]; then
    FGVC_DATA_ROOT=$CURDIR/dataset/$FGVC_DATASET
fi

if [ -z "$PRETRAIN_DATA_PATH" ]; then
    if [[ $TYPE =~ cub_pretrain ]] || [[ $TYPE =~ air_pretrain ]] || [[ $TYPE =~ car_pretrain ]] || [[ $TYPE =~ cars_pretrain ]] || [[ $TYPE =~ fgvc_pretrain ]]; then
        PRETRAIN_DATA_PATH=${FGVC_PRETRAIN_DATA_PATH:-${FGVC_DATA_ROOT}/${PRETRAIN_SPLIT:-train}}
    else
        PRETRAIN_DATA_PATH=data/imagenet/train
    fi
fi

if [ -z "$LINEAR_DATA_PATH" ]; then
    if [[ $TYPE =~ cub_linear ]] || [[ $TYPE =~ air_linear ]] || [[ $TYPE =~ car_linear ]] || [[ $TYPE =~ cars_linear ]] || [[ $TYPE =~ fgvc_linear ]]; then
        LINEAR_DATA_PATH=${FGVC_LINEAR_DATA_PATH:-$FGVC_DATA_ROOT}
    else
        LINEAR_DATA_PATH=data/imagenet
    fi
fi

PRETRAIN_EXTRA_ARGS=()
if [ ! -z "$INIT_PRETRAINED" ]; then
    PRETRAIN_EXTRA_ARGS+=(--pretrained_weights "$INIT_PRETRAINED")
    PRETRAIN_EXTRA_ARGS+=(--pretrained_checkpoint_key "${INIT_CHECKPOINT_KEY:-teacher}")
fi
...
if [[ $TYPE =~ imagenet_knn ]] || [[ $TYPE =~ imagenet_reg ]] || \
   [[ $TYPE =~ imagenet_linear ]] || [[ $TYPE =~ cub_linear ]] || [[ $TYPE =~ air_linear ]] || [[ $TYPE =~ car_linear ]] || [[ $TYPE =~ cars_linear ]] || [[ $TYPE =~ fgvc_linear ]] || [[ $TYPE =~ imagenet_cls ]] || \
   ...
   [[ $TYPE =~ davis_viseg ]] || [[ $TYPE =~ paris_oxford_reid ]] || [[ $TYPE =~ copydays_copydet ]]; then
    ...
    if [[ $TYPE =~ cub_linear ]] || [[ $TYPE =~ air_linear ]] || [[ $TYPE =~ car_linear ]] || [[ $TYPE =~ cars_linear ]] || [[ $TYPE =~ fgvc_linear ]]; then
        FGVC_TAG=${FGVC_DATASET:-fgvc}
        for K in $(seq ${NODE_ID:-0} ${TOTAL_NODES:-1} $[${#KEY_LIST[@]}-1]) 
        do
            if [[ $TYPE =~ cub_linear_solo ]] || [[ $TYPE =~ air_linear_solo ]] || [[ $TYPE =~ car_linear_solo ]] || [[ $TYPE =~ cars_linear_solo ]] || [[ $TYPE =~ fgvc_linear_solo ]]; then
                SUB_OUTPUT_DIR=$OUTPUT_DIR/${FGVC_TAG}_linear_solo
                python3 -m torch.distributed.launch --nproc_per_node=$GPUS_PER_NODE \
                    --master_port=$[${MASTER_PORT:-29500}-$K] \
                    ${CURDIR}/evaluation/eval_linear.py \
                    --pretrained_weights $PRETRAINED \
                    ...
                    --output_dir $SUB_OUTPUT_DIR \
                    --data_path $LINEAR_DATA_PATH \
                    --train_split ${LINEAR_TRAIN_SPLIT:-train} \
                    --val_split ${LINEAR_VAL_SPLIT:-test} \
                    --num_labels ${LINEAR_NUM_LABELS:-0} \
                    ${@:6}
            else
                SUB_OUTPUT_DIR=$OUTPUT_DIR/${FGVC_TAG}_linear
                python3 -m torch.distributed.launch --nproc_per_node=$GPUS_PER_NODE \
                    --master_port=$[${MASTER_PORT:-29500}-$K] \
                    $CURDIR/evaluation/eval_linear_multi.py \
                    --pretrained_weights $PRETRAINED \
